...
DEFAULT_FOLDER = Path.home() / "C:/Users/User/Desktop/Real lens propagation"

# Falls du die Pixelgröße kennst, hier eintragen, z.B. 4.65 für 4.65 µm/Pixel.
...
def choose_pixel_offset(frame_bytes: bytes, width: int, height: int, candidates: list[int]) -> int:
    ...
    Heuristik:
    - möglichst wenig Signal am Rand
    - Maximum soll nicht direkt am Rand liegen
    """
    best_offset = None
    best_score = np.inf
...
def read_wcf_mean_image(path: Path) -> np.ndarray:
    """
    Liest eine DataRay-artige WCF-Datei (IRD1-Header) und mittelt alle nutzbaren Frames.
    """
...
def subtract_background(img):
    border = min(40, img.shape[0] // 8)

    top = img[:border, :]
    bottom = img[-border:, :]

    bg = bg = np.percentile(img, 8)
...
def second_moment_diameter_avg(img: np.ndarray) -> dict[str, float]:
    """
    Gemittelter D4σ-Durchmesser aus x- und y-Richtung:
    D4sigma_avg = (D4sigma_x + D4sigma_y) / 2
    """
    total = float(img.sum())
    y, x = np.indices(img.shape)
...
def maybe_scale(values: dict[str, float]) -> tuple[dict[str, float], str]:
    ...
    for k, v in values.items():
        scaled[k] = v * PIXEL_SIZE_UM / 1000.0  # µm -> mm
    return scaled, "mm"
...
def main(folder: Path = DEFAULT_FOLDER) -> None:
    ...
    for path in files:
        ...
        metrics, unit = maybe_scale(metrics)

        row = {"file": path.name, "z_cm": z_cm}
        row.update(metrics)
        rows.append(row)
    ...
    # CSV speichern
    csv_path = folder / "beam_size_vs_z.csv"
    ...
    # Plot mit genau 5 gemittelten Size-Definitionen
    ...
    plt.title("Laser beam size in propagation direction")
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()

    plot_path = folder / "beam_size_vs_z.png"
