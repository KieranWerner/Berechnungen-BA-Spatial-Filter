...
# Output folder for the single-parameter optimization
OUTPUT_DIR = Path(r"C:\Users\User\Desktop\single_parameter_optimization")
...
# Reproduzierbarkeit für zufällige Effekte
RANDOM_SEED = 12345
...
# -----------------------------
# Single-parameter optimization settings
# -----------------------------
N_VALUES = 100
...
PARAMETERS = [
    {
        ...
        "config_key": "DEFOCUS",
        "values": np.linspace(-0.8884 * 1.5, +0.8884 * 1.5, N_VALUES),
    },
    {
        "name": "TILT_X",
        "use_flags": {"USE_EXTRA_PHASE": True},
        "config_key": "TILT_X",
        "values": np.linspace(-0.003, +0.003, N_VALUES),
    },
    {
        "name": "TILT_Y",
        "use_flags": {"USE_EXTRA_PHASE": True},
    ...
    },
    {
        ...
        "use_flags": {"USE_EXTRA_PHASE": True},
        "config_key": "ASTIG_X",
        "values": np.linspace(-0.5, +0.5, N_VALUES),
    },
    {
        "name": "ASTIG_Y",
        "use_flags": {"USE_EXTRA_PHASE": True},
    ...
    },
    {
        ...
        "values": np.linspace(0.0, 2.5, N_VALUES),
    },
    {
        "name": "Z_OFFSET_M",
        "use_flags": {"USE_Z_OFFSET": True},
        "config_key": "Z_OFFSET_M",
        "values": np.linspace(-0.30, +0.30, N_VALUES),
    },
    {
...
def run_theory_for_scenario(output_dir: Path):
    ...
    U0_without_phase = apply_extra_phase(U0_without_phase, dx_m, dy_m, wavelength_medium)
    ...
    plot_rows = []
    for i, z in enumerate([r["z_m"] for r in rows_with_phase]):
        plot_rows.append({
            "z_m": z,
...
def evaluate_single_value(df_meas: pd.DataFrame, parameter_def: dict, value: float, index: int):
    global CFG
    ...
    scenario_dir = OUTPUT_DIR / parameter_def["name"] / scenario_name
...
def optimize_one_parameter(df_meas: pd.DataFrame, parameter_def: dict):
    ...
    plt.axvline(best_row["parameter_value"], linestyle="--", linewidth=1, label=f"Best {RANK_BY}")
...
def main():
    ...
    df_best.to_csv(OUTPUT_DIR / "best_values_per_parameter.csv", index=False)
    df_best_sorted.to_csv(OUTPUT_DIR / "best_values_per_parameter_sorted.csv", index=False)

    plt.figure(figsize=(12, 6))
    plt.bar(df_best_sorted["parameter"], df_best_sorted[RANK_BY])
    plt.xticks(rotation=45, ha="right")
    plt.ylabel(f"{RANK_BY} [mm]")
    plt.title(f"Best single value per parameter ranked by {RANK_BY}")
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "best_parameter_ranking.png", dpi=200)
    plt.close()

    print("\n" + "=" * 100)
    print("FINAL BEST VALUES PER PARAMETER")
    print("=" * 100)
