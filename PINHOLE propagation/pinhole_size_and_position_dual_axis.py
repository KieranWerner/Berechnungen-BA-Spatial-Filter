...
CFG = {
    ...
    # Output
    "OUTPUT_DIR": Path(r"C:\Users\User\Desktop\PINHOLE_fast_dual_axis"),
    ...
    # --------------------------------------------------------
    # Fixed pinhole for diameter sweep
    # --------------------------------------------------------
    ...
    # Faster sweeps
    # --------------------------------------------------------
    "PINHOLE_DIAMETER_MIN_M": 500e-6,
    "PINHOLE_DIAMETER_MAX_M": 1800e-6,
    "PINHOLE_DIAMETER_NUM_POINTS": 10,

    "PINHOLE_POSITION_MIN_M": 7.40,
    "PINHOLE_POSITION_MAX_M": 7.80,
    "PINHOLE_POSITION_NUM_POINTS": 12,

    "PINHOLE_FIXED_DIAMETER_FOR_POSITION_SWEEP_M": 1106e-6,

    "POST_LENS2_EVAL_Z_MIN_REL_M": 0.006,
    "POST_LENS2_EVAL_Z_MAX_REL_M": 0.020,
    "POST_LENS2_EVAL_Z_NUM_POINTS": 12,
...
def angular_spectrum(U0: np.ndarray, dx: float, dy: float, wavelength: float, z: float, pad_factor: int = 1) -> np.ndarray:
    ny, nx = U0.shape
...
def save_dual_axis_plot(
    ...
    l1 = ax1.plot(x_vals, y_left, marker="o", linewidth=2, label=y_left_label)
    ax1.set_xlabel(xlabel)
    ax1.set_ylabel(y_left_label)
    ax1.grid(True, alpha=0.3)

    ax2 = ax1.twinx()
    l2 = ax2.plot(x_vals, y_right, marker="s", linewidth=2, linestyle="--", label=y_right_label)
...
def fixed_pinhole_size_evalz_sweep(
    U0: np.ndarray,
    dx_m: float,
    dy_m: float,
    wavelength: float,
    ...
    for i_d, pinhole_d in enumerate(pinhole_d_values):
        ...
        for i_z, z_eval in enumerate(z_eval_values):
            ...
            if np.isfinite(gq["gaussian_overlap_2d"]) and gq["gaussian_overlap_2d"] > best_overlap_global:
                best_overlap_global = gq["gaussian_overlap_2d"]
                best_result = row
            ...
            if case_counter % max(1, int(CFG["PRINT_EVERY_N_STEPS"])) == 0 or case_counter == total_cases:
                ...
                msg = (
                    f"Diameter sweep: {case_counter}/{total_cases}  "
                    f"d={pinhole_d * 1e6:.1f} um, "
                    f"z_eval={z_eval:.6f} m, "
                    f"trans={transmission:.4f}, "
                    f"overlap={gq['gaussian_overlap_2d']:.5f}"
                )
                if np.isfinite(remaining):
                    msg += f", ETA={remaining:.1f} s"
                log(msg)

    csv_path = output_dir / "diameter_sweep_transmission_quality.csv"
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=list(rows[0].keys()))
        writer.writeheader()
        writer.writerows(rows)

    json_path = output_dir / "diameter_sweep_transmission_quality.json"
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump(rows, f, indent=2)

    d_um = pinhole_d_values * 1e6

    best_trans_vs_d = np.full(nd, np.nan, dtype=float)
    best_gq_vs_d = np.full(nd, np.nan, dtype=float)

    for i_d in range(nd):
        if np.all(~np.isfinite(overlap_map[:, i_d])):
            continue
        i_best = int(np.nanargmax(overlap_map[:, i_d]))
        best_trans_vs_d[i_d] = trans_map[i_best, i_d]
        best_gq_vs_d[i_d] = overlap_map[i_best, i_d]

    save_dual_axis_plot(
        x_vals=d_um,
        y_left=best_trans_vs_d,
        y_right=best_gq_vs_d,
        xlabel="Pinhole diameter [µm]",
        y_left_label="Transmission",
        y_right_label="Gaussian overlap (2D)",
        title=f"Transmission and Gaussian quality vs pinhole diameter\n(fixed pinhole z = {pinhole_z * 1e3:.1f} mm)",
        out_png=output_dir / "transmission_and_gaussian_quality_vs_pinhole_diameter.png",
        out_pdf=output_dir / "transmission_and_gaussian_quality_vs_pinhole_diameter.pdf",
    )
...
def pinhole_position_evalz_sweep(
    U0: np.ndarray,
    dx_m: float,
    dy_m: float,
    wavelength: float,
    ...
    pinhole_positions = np.linspace(
        float(CFG["PINHOLE_POSITION_MIN_M"]),
        float(CFG["PINHOLE_POSITION_MAX_M"]),
        int(CFG["PINHOLE_POSITION_NUM_POINTS"]),
    )

    fixed_diameter = float(CFG["PINHOLE_FIXED_DIAMETER_FOR_POSITION_SWEEP_M"])
    ...
    best_trans_vs_pos = np.full(len(pinhole_positions), np.nan, dtype=float)
    best_gq_vs_pos = np.full(len(pinhole_positions), np.nan, dtype=float)
    ...
    total_cases = len(pinhole_positions) * len(z_eval_values)
    case_counter = 0
    sweep_t0 = time.perf_counter()

    for i_p, pinhole_z in enumerate(pinhole_positions):
        U_before_pinhole = propagate_to_z_with_elements(
            U_start=U0,
            dx=dx_m,
            dy=dy_m,
        ...
        local_best_overlap = -np.inf
        local_best_trans = np.nan
        ...
        for z_eval in z_eval_values:
            ...
            rows.append({
                ...
                "pinhole_diameter_m": float(fixed_diameter),
                "evaluation_z_m": float(z_eval),
                "transmission": float(transmission),
                "gaussian_overlap_2d": float(gq["gaussian_overlap_2d"]),
            })

            if np.isfinite(gq["gaussian_overlap_2d"]) and gq["gaussian_overlap_2d"] > local_best_overlap:
                local_best_overlap = gq["gaussian_overlap_2d"]
                local_best_trans = transmission
            ...
            if case_counter % max(1, int(CFG["PRINT_EVERY_N_STEPS"])) == 0 or case_counter == total_cases:
                ...
                msg = (
                    f"Position sweep: {case_counter}/{total_cases}  "
                    f"z_p={pinhole_z * 1e3:.1f} mm, "
                    f"z_eval={z_eval:.6f} m, "
                    f"trans={transmission:.4f}, "
                    f"overlap={gq['gaussian_overlap_2d']:.5f}"
                )
        ...
        if np.isfinite(local_best_overlap):
            best_trans_vs_pos[i_p] = local_best_trans
            best_gq_vs_pos[i_p] = local_best_overlap

    csv_path = output_dir / "position_sweep_transmission_quality.csv"
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=list(rows[0].keys()))
        writer.writeheader()
        writer.writerows(rows)

    json_path = output_dir / "position_sweep_transmission_quality.json"
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump(rows, f, indent=2)

    x_mm = pinhole_positions * 1e3

    save_dual_axis_plot(
        x_vals=x_mm,
        y_left=best_trans_vs_pos,
        y_right=best_gq_vs_pos,
        xlabel="Pinhole position [mm]",
        y_left_label="Transmission",
        y_right_label="Gaussian overlap (2D)",
        title=f"Transmission and Gaussian quality vs pinhole position\n(fixed diameter = {fixed_diameter * 1e6:.1f} µm)",
        out_png=output_dir / "transmission_and_gaussian_quality_vs_pinhole_position.png",
        out_pdf=output_dir / "transmission_and_gaussian_quality_vs_pinhole_position.pdf",
    )
...
def main() -> None:
    ...
    diameter_result = fixed_pinhole_size_evalz_sweep(
        U0=U0,
        dx_m=dx_m,
        dy_m=dy_m,
        wavelength=wavelength,
    ...
    position_result = pinhole_position_evalz_sweep(
        U0=U0,
        dx_m=dx_m,
        dy_m=dy_m,
        wavelength=wavelength,
    ...
    if CFG["SAVE_BEST_INTENSITY_MAP"]:
        candidates = []
        if diameter_result["best_result"] is not None:
            candidates.append((diameter_result["best_result"], diameter_result["best_intensity"]))
        if position_result["best_result"] is not None:
            candidates.append((position_result["best_result"], position_result["best_intensity"]))

        if candidates:
            best_pair = max(candidates, key=lambda x: x[0]["gaussian_overlap_2d"])
            best_result, best_intensity = best_pair

            save_best_intensity_map(
                best_intensity=best_intensity,
                dx_m=dx_m,
                dy_m=dy_m,
                title=(
                ...
                out_png=output_dir / "best_result_intensity_map.png",
                out_pdf=output_dir / "best_result_intensity_map.pdf",
            )
    ...
    meta = {
        ...
        "pinhole_position_num_points": int(CFG["PINHOLE_POSITION_NUM_POINTS"]),
    ...
    print()
    print("=" * 80)
    print("TRANSMISSION + GAUSSIAN QUALITY SWEEPS FINISHED")
    print("=" * 80)
    print(f"Output directory:                    {output_dir}")
    print(f"Pixel size in theory:                {dx_m * 1e6:.6f} um")
    print(f"Diameter points:                     {CFG['PINHOLE_DIAMETER_NUM_POINTS']}")
    print(f"Position points:                     {CFG['PINHOLE_POSITION_NUM_POINTS']}")
    print(f"Eval z points:                       {CFG['POST_LENS2_EVAL_Z_NUM_POINTS']}")
    print(f"Total runtime:                       {total_elapsed:.2f} s")
    print()
    print("Created plots:")
    print("  - transmission_and_gaussian_quality_vs_pinhole_diameter.png")
    print("  - transmission_and_gaussian_quality_vs_pinhole_position.png")
    print("  - best_result_intensity_map.png")
