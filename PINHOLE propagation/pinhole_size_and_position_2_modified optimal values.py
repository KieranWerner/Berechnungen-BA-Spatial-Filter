...
CFG = {
    ...
    # Output
    "OUTPUT_DIR": Path(r"C:\Users\User\Desktop\PINHOLE_optimal values"),
    ...
    "APPLY_SPHERICAL_PHASE": False,
    "SPHERICAL_COEFF_M_INV3": 0.0,

    "APPLY_COMA_PHASE": False,
    "COMA_X_M_INV2": 0.0,
    "COMA_Y_M_INV2": 0.0,

    # Optional global z shift loaded from optimization file
    "GLOBAL_Z_OFFSET_M": 0.0,

    # Optional external file with optimized parameters
    "APPLY_OPTIMAL_VALUES_FILE": True,
    "OPTIMAL_VALUES_FILE": Path(r"C:\Users\User\Desktop\999 fertige Dateien\best_values_per_parameter.csv"),
    ...
    # --------------------------------------------------------
    "PINHOLE_Z_LOCAL_HALF_RANGE_M": 2.00,
    "PINHOLE_Z_NUM_POINTS": 9,

    "PINHOLE_DIAMETER_MIN_M": 50e-6,
    "PINHOLE_DIAMETER_MAX_M": 2000e-6,
    "PINHOLE_DIAMETER_NUM_POINTS": 50,
...
def load_best_parameter_values(path: Path) -> dict[str, float]:
    if not path.exists():
        raise FileNotFoundError(f"Optimized-parameter file not found: {path}")
    ...
    with open(path, "r", encoding="utf-8-sig", newline="") as f:
        ...
        required = {"parameter", "parameter_value"}
        if reader.fieldnames is None or not required.issubset(set(reader.fieldnames)):
            raise ValueError(
                "Parameter CSV must contain at least the columns "
        ...
        for row in reader:
            ...
            try:
                ...
                raise ValueError(
                    f"Invalid parameter_value for parameter {key!r}: {row['parameter_value']!r}"
                ) from exc
            result[key] = value
...
def apply_best_parameter_values_to_cfg(best_values: dict[str, float]) -> list[str]:
    unsupported: list[str] = []

    if "DEFOCUS" in best_values:
        CFG["APPLY_EXTRA_DEFOCUS_PHASE"] = True
        CFG["DEFOCUS_COEFF_M_INV"] = float(best_values["DEFOCUS"])

    if "TILT_X" in best_values or "TILT_Y" in best_values:
        CFG["APPLY_TILT_PHASE"] = True
        CFG["TILT_X_RAD_PER_M"] = float(best_values.get("TILT_X", CFG["TILT_X_RAD_PER_M"]))
        CFG["TILT_Y_RAD_PER_M"] = float(best_values.get("TILT_Y", CFG["TILT_Y_RAD_PER_M"]))

    if "ASTIG_X" in best_values or "ASTIG_Y" in best_values:
        CFG["APPLY_ASTIGMATISM_PHASE"] = True
        CFG["ASTIG_X_M_INV"] = float(best_values.get("ASTIG_X", CFG["ASTIG_X_M_INV"]))
        CFG["ASTIG_Y_M_INV"] = float(best_values.get("ASTIG_Y", CFG["ASTIG_Y_M_INV"]))

    if "SPHERICAL_COEFF" in best_values:
        CFG["APPLY_SPHERICAL_PHASE"] = True
        CFG["SPHERICAL_COEFF_M_INV3"] = float(best_values["SPHERICAL_COEFF"])

    if "COMA_X" in best_values or "COMA_Y" in best_values:
        CFG["APPLY_COMA_PHASE"] = True
        CFG["COMA_X_M_INV2"] = float(best_values.get("COMA_X", CFG["COMA_X_M_INV2"]))
        CFG["COMA_Y_M_INV2"] = float(best_values.get("COMA_Y", CFG["COMA_Y_M_INV2"]))

    if "Z_OFFSET_M" in best_values:
        CFG["GLOBAL_Z_OFFSET_M"] = float(best_values["Z_OFFSET_M"])

    for name in best_values:
        if name not in {
            "DEFOCUS", "TILT_X", "TILT_Y",
            "ASTIG_X", "ASTIG_Y",
            "SPHERICAL_COEFF", "COMA_X", "COMA_Y",
            "Z_OFFSET_M",
        }:
...
def apply_gaussian_blur_if_requested(arr: np.ndarray, sigma_px: float) -> np.ndarray:
    ...
    try:
        ...
        blurred = np.asarray(arr, dtype=float).copy()
        for _ in range(max(1, int(round(sigma_px)))):
            blurred = (
...
def apply_start_plane_phase(U0: np.ndarray, dx: float, dy: float, wavelength: float) -> np.ndarray:
    ...
    if CFG["APPLY_SPHERICAL_PHASE"]:
        r2 = X**2 + Y**2
        phi += k * float(CFG["SPHERICAL_COEFF_M_INV3"]) * (r2**2)

    if CFG["APPLY_COMA_PHASE"]:
        phi += k * (
            float(CFG["COMA_X_M_INV2"]) * (X**3 + X * Y**2) +
            float(CFG["COMA_Y_M_INV2"]) * (Y**3 + Y * X**2)
        )
...
def build_elements_without_pinhole() -> list[dict]:
    elements: list[dict] = []
    z_shift = float(CFG["GLOBAL_Z_OFFSET_M"])

    if CFG["LENS1_ENABLED"]:
        elements.append({
            "type": "lens",
            "z_m": float(CFG["LENS1_Z_M"]) + z_shift,
    ...
    if CFG["LENS2_ENABLED"]:
        elements.append({
            "type": "lens",
            "z_m": float(CFG["LENS2_Z_M"]) + z_shift,
...
def find_nominal_pinhole_plane(
    ...
    z_values = np.linspace(
        float(CFG["PINHOLE_NOMINAL_SEARCH_Z_MIN_M"]) + float(CFG["GLOBAL_Z_OFFSET_M"]),
        float(CFG["PINHOLE_NOMINAL_SEARCH_Z_MAX_M"]) + float(CFG["GLOBAL_Z_OFFSET_M"]),
...
def fast_pinhole_sweep() -> None:
    ...
    if CFG["APPLY_OPTIMAL_VALUES_FILE"]:
        log("Lade optimierte Parameter ...")
        best_values = load_best_parameter_values(Path(CFG["OPTIMAL_VALUES_FILE"]))
        unsupported_parameters = apply_best_parameter_values_to_cfg(best_values)
        log(f"Geladene Parameter: {best_values}")
        if unsupported_parameters:
            log(
                "Hinweis: Diese Parameter wurden aus der CSV gelesen, aber im Modell "
                f"nicht direkt verwendet: {unsupported_parameters}"
            )
    else:
        best_values = {}
    ...
    if "BLUR_SIGMA_PX" in best_values and float(best_values["BLUR_SIGMA_PX"]) > 0:
        log(f"Wende Gauß-Blur auf Eingangsintensität an: sigma = {float(best_values['BLUR_SIGMA_PX']):.6f} px")
        intensity = apply_gaussian_blur_if_requested(intensity, float(best_values["BLUR_SIGMA_PX"]))
    ...
    if CFG["USE_EVAL_MINI_SWEEP"]:
        z_eval_values = np.linspace(
            float(CFG["LENS2_Z_M"]) + float(CFG["GLOBAL_Z_OFFSET_M"]) + float(CFG["POST_LENS2_EVAL_Z_MIN_REL_M"]),
            float(CFG["LENS2_Z_M"]) + float(CFG["GLOBAL_Z_OFFSET_M"]) + float(CFG["POST_LENS2_EVAL_Z_MAX_REL_M"]),
        ...
        z_eval_values = np.array([
            float(CFG["LENS2_Z_M"]) + float(CFG["GLOBAL_Z_OFFSET_M"]) + float(CFG["GAUSSIAN_QUALITY_AFTER_LENS2_DELTA_Z_M"])
        ])
    ...
    meta = {
        ...
        "loaded_best_parameters": best_values,
        "parameters_loaded_but_unused": unsupported_parameters,
