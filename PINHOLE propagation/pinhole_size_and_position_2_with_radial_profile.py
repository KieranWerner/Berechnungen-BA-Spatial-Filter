...
CFG = {
    ...
    "SAVE_BEST_RADIAL_PROFILE": True,
    "RADIAL_PROFILE_NUM_BINS": None,
...
def radial_profile(I: np.ndarray, dx: float, dy: float, nbins: int | None = None) -> dict[str, np.ndarray]:
    I = np.clip(np.asarray(I, dtype=float), 0.0, None)
    if I.ndim != 2:
        raise ValueError("radial_profile expects a 2D intensity map.")
    ...
    dr = max(abs(dx), abs(dy))
    if dr <= 0:
        raise ValueError("dx and dy must be positive.")

    if nbins is None:
        nbins = max(10, int(np.ceil(r_max / dr)))
    ...
    sums, _ = np.histogram(r_flat, bins=edges, weights=i_flat)

    with np.errstate(invalid='ignore', divide='ignore'):
        profile_mean = sums / counts

    profile_mean[counts == 0] = np.nan

    finite = np.isfinite(profile_mean)
    if np.any(finite):
        norm = np.nanmax(profile_mean)
        if np.isfinite(norm) and norm > 0:
            profile_norm = profile_mean / norm
        else:
    ...
    return {
        "radius_m": centers,
        "radius_mm": centers * 1e3,
        "mean_intensity": profile_mean,
        "normalized_mean_intensity": profile_norm,
        "counts": counts.astype(float),
        "center_x_px": np.array([x0_px], dtype=float),
        "center_y_px": np.array([y0_px], dtype=float),
    }
...
def save_radial_profile_plot(
    ...
    valid = np.isfinite(radius_mm) & np.isfinite(profile_norm)

    plt.figure(figsize=(8.5, 5.5))
    plt.plot(radius_mm[valid], profile_norm[valid])
    plt.xlabel("Radius [mm]")
    plt.ylabel("Normalized radial intensity")
    plt.title("Radial profile of best result")
...
def fast_pinhole_sweep() -> None:
    ...
    if CFG["SAVE_BEST_RADIAL_PROFILE"] and best_intensity is not None and best_result is not None:
        radial = radial_profile(
            best_intensity,
            dx=dx_m,
            dy=dy_m,
            nbins=CFG["RADIAL_PROFILE_NUM_BINS"],
        )

        radial_csv_path = output_dir / "best_result_radial_profile.csv"
        with open(radial_csv_path, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(["radius_m", "radius_mm", "mean_intensity", "normalized_mean_intensity", "counts"])
            for r_m, r_mm, mean_i, norm_i, cnt in zip(
                radial["radius_m"],
                radial["radius_mm"],
                radial["mean_intensity"],
                radial["normalized_mean_intensity"],
                radial["counts"],
            ):
                writer.writerow([r_m, r_mm, mean_i, norm_i, cnt])

        save_radial_profile_plot(
            radial["radius_mm"],
            radial["normalized_mean_intensity"],
            out_png=output_dir / "best_result_radial_profile.png",
            out_pdf=output_dir / "best_result_radial_profile.pdf",
        )

        radial_meta = {
            "center_x_px": float(radial["center_x_px"][0]),
            "center_y_px": float(radial["center_y_px"][0]),
            "num_bins": int(len(radial["radius_m"])),
            "csv_path": str(radial_csv_path),
        }
    else:
        radial_meta = None
    ...
    meta = {
        ...
        "best_radial_profile": radial_meta,
    }
    ...
    if CFG["SAVE_BEST_RADIAL_PROFILE"]:
        print(f"Radial profile CSV:                  {output_dir / 'best_result_radial_profile.csv'}")
