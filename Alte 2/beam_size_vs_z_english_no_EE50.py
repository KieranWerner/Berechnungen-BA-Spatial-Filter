...
PIXEL_SIZE_UM = None   # set e.g. 5.2 if pixel size is known

# candidate offsets found from sample WCF file
CANDIDATE_PIXEL_OFFSETS = [
    9348,
    13924,
    20000,
    25000
...
# save CSV
csv_path = os.path.join(FOLDER, "beam_size_vs_z.csv")
...
for col in df.columns:
    if col != "z_cm":
        plt.plot(df["z_cm"], df[col], marker="o", label=col)

plt.xlabel("z [cm]")
plt.ylabel(ylabel)
plt.title("Laser beam size along z")
plt.legend()
plt.grid(True)

png_path = os.path.join(FOLDER, "beam_size_vs_z.png")
...
png2_path = os.path.join(FOLDER, "beam_size_vs_z_d4sigma.png")
