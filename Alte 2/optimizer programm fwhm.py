...
BASE_OUTPUT_DIR = Path(r"C:\Users\User\Desktop\extended_parameter_optimization_fwhm")
...
MEAS_OUTPUT_DIR = MEAS_BASE_DIR / "results_fwhm"
MEAS_OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
...
def run_theory_for_scenario(int_file: Path, output_dir: Path):
    ...
    input_fwhm_px = tmp_px["fwhm_avg"]

    if not np.isfinite(input_fwhm_px) or input_fwhm_px <= 0:
        raise ValueError("Input FWHM could not be determined from the input intensity.")

    dx_m = (INPUT_FWHM_MM / input_fwhm_px) * 1e-3
    dy_m = dx_m
...
def run_measurement():
    ...
    plot_csv_path = MEAS_OUTPUT_DIR / "beam_size_vs_position_plot_data.csv"
    ...
    plot_json_path = MEAS_OUTPUT_DIR / "beam_size_vs_position_plot_data.json"
    with open(plot_json_path, "w", encoding="utf-8") as f:
        json.dump(plot_rows, f, indent=2)

    full_csv_path = MEAS_OUTPUT_DIR / "beam_size_vs_position_full_results.csv"
    ...
    out_path = MEAS_OUTPUT_DIR / "beam_size_vs_position.png"
...
def main():
    ...
    print(f"Measurement output: {MEAS_OUTPUT_DIR}")
